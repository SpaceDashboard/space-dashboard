...
if (iOS) {
  rootDoc.className += ' ios';
}

if (window.location.hostname === 'localhost') {
  // dev variables - 30 seconds for local testing
  autoReload = false;
...
function loadIssFeed1(autoplay) {
  ...
  $('.iss1-player-type[data-type="' + localStorage.iss1Type + '"]').addClass(
    'active',
  );

  var $iss1 =
    '<iframe class="iss-feed-1" id="iss-feed-1" width="560" height="315" src="https://www.youtube.com/embed/1rcSyN74BY0?&autoplay=1" title="YouTube video player - ISS Live View" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>';
...
function loadIssFeed2(html5, autoplay) {
  ...
  var $iss2 =
    '<iframe class="iss-feed-2" id="iss-feed-2" src="https://www.youtube.com/embed/K5JSRGhB-nM?&autoplay=1" title="YouTube video player - Live High-Definition Views from ISS" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>';
...
function loadNASATV(html5, autoplay) {
  ...
  var $NASATV =
    '<iframe class="iss-feed-2" id="iss-feed-2" src="https://www.youtube.com/embed/21X5lGlDOfg?&autoplay=1" title="YouTube video player - NASA TV" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowfullscreen></iframe>';
    ...
    if (!$(this).is(':hidden')) {
      ...
      if ($(this)) {
        if (tagEl === 'VIDEO') {
          ...
          var source =
            '<source src="https://api.spacedashboard.com/vid/current-corona.mp4?lastrefresh=' +
            seconds_since_epoch() +
            '" type="video/mp4">';
          $(this).html(source);
        } else {
          var elementSrcReference = $(this)
            .attr('src')
            .replace(/\?lastrefresh=.*/, '');
    ...
    if (xhr.status === 200) {
      try {
        ...
        document.getElementById('neo-list').innerHTML =
          '<div class="error-text-fetching-data">Error retrieving near-Earth objects</div>';
      }
    }
  };
  xhr.onerror = function (e) {
    document.getElementById('text-aurora-forecast').innerHTML =
      '<div class="error-text-fetching-data">Error retrieving aurora forecast</div>';
    if (Callbacks) {
      Callbacks();
    }
  };
  ...
  xhr.onload = function (e) {
    if (xhr.status === 200) {
      try {
        ...
        console.log(error);
        document.getElementById('people-in-space').innerHTML =
          '<div class="error-text-fetching-data">Error retrieving people in space</div>';
      }
    } else {
  ...
  _.each(people, function (val, key) {
    ...
    if (key === 'people') {
      _.each(people.people, function (obj) {
        const countryName =
          obj.country !== '' ? obj.country : obj.countryflag.slice(58, -4);
        ...
        const capitalizedCountryName =
          firstLetterCountryUpperCase + remainingCountryName;
...
$(function () {
  ...
  $('.toggle-solar-visual').on('click', function () {
    ...
    if (
      solarActivity === 'video' &&
      $('.solar-activity[data-solar-activity="video"] source').length === 0
    ) {
      var source =
        '<source src="https://api.spacedashboard.com/vid/current-corona.mp4" type="video/mp4">';
  ...
  $('.toggle-hemisphere').on('click', function () {
    ...
    $(
      '.img-aurora-forecast[data-hemisphere="' + hemisphereForecast + '"]',
    ).show();

    $('.toggle-hemisphere').show();
    $(
      '.toggle-hemisphere[data-hemisphere="' + hemisphereForecast + '"]',
    ).hide();
  });
      ...
      success: function (data, xhr) {
        if (xhr === 'success' && data === 'Sentient check failed') {
          $('#contact-form .text-error')
            .text('Human check failed, please uncheck the checkbox.')
            .show();
        } else if (xhr === 'success' && data === 'Email is required') {
          $('#contact-form .text-error')
            .text('Sorry, an email is required.')
            .show();
