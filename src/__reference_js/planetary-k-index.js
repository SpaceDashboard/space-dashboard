...
function fetchCurrentKindex() {
  ...
  xhr.open(
    'GET',
    'https://api.spacedashboard.com/json/planetary-k-index-dst.json',
  );
  xhr.onload = function (e) {
    if (xhr.status === 200) {
      try {
        ...
        var localTime = moment(moment.tz(latestKindex['time_tag'], 'UTC'))
          .tz(moment.tz.guess())
          .format('HH:mm');
        ...
        if (!latestKindex) {
          ...
          $('.kindex-local-timestamp')
            .empty()
            .append('--:-- ' + localTimezone);
        ...
        $('.k-index-value')
          .empty()
          .append(latestKindex['estimated_kp'].toFixed(2));
        $('.kindex-local-timestamp')
          .empty()
          .append(localTime + ' ' + localTimezone);
        $('.kindex-utc-timestamp')
          .empty()
          .append(utcTime + ' UTC');
        if (storm !== 'G0') {
          $('.geo-storm').append(storm).show();
        }
...
function fetchThreeHourKindex(updateChart, Callbacks) {
  ...
  xhr.open(
    'GET',
    'https://api.spacedashboard.com/json/noaa-planetary-k-index.json',
  );
  xhr.onload = function (e) {
    if (xhr.status === 200) {
      try {
        ...
        kIndexData.kIndexTooltipLabelArray =
          kIndexData.kIndexTooltipLabelArray.slice(-18);
        kIndexData.kIndexTooltipDateArray =
          kIndexData.kIndexTooltipDateArray.slice(-18);
        ...
        console.log(error);
        document.querySelector('.k-index-wrapper').innerHTML =
          '<div class="historical-k-index-error error-text-fetching-data">Error retrieving historical planetary K index</div>';
        if (Callbacks) {
          Callbacks();
        }
      }
...
function renderThreeHourKindex(indexData, updateChart) {
  ...
  $('.k-index-wrapper')
    .empty()
    .append('<canvas id="planetary-k-index"></canvas>');
      ...
      tooltips: {
        ...
        callbacks: {
          ...
          label: function (tooltipItem, data) {
            var label =
              data.datasets[0].tooltipLabels[tooltipItem.index] +
              ', \n' +
