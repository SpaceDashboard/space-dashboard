    ...
    if (xhr.status === 200) {
      if (xhr.response === 'Request failed') {
        ...
        try {
          ...
          var cleanResponse = JSON.parse(
            response.substring(response.indexOf('{"links')),
          );
...
function renderNEOs(neoArray) {
  ...
  var neoInfoWithHeader =
    '<h2>Near Earth Objects <small>(in lunar distance)</small></h2>' +
    finalNeoInfo;
