...
function checkSiteMessages() {
  ...
  xhr.onload = function (e) {
    if (xhr.status === 200) {
      ...
      var timeNowRoundedNearestMinute = moment()
        .utc()
        .startOf('minute')
      ...
      if (webcastStartTime !== '' && webcastEndTime !== '') {
        ...
        if (
          timeNowRoundedNearestMinute >= webcastStartTime &&
          timeNowRoundedNearestMinute < webcastEndTime
        ) {
          showWebcast = true;
        }
      }
       ...
       */
      webcastObj['embed_code'] = webcastObj.use_NASA_TV
        ? webcastObj.NASA_TV_embed_code
        : webcastObj.custom_embed_code;
